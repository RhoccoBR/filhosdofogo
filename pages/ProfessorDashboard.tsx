      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#3b0764] via-[#581c87] to-[#1e1b4b] shadow-2xl border border-purple-500/20 isolate group">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-start gap-6 p-6 sm:p-8">
            <div className="relative shrink-0">
                <div 
                    className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-full p-1 bg-gradient-to-tr from-orange-500 via-purple-500 to-blue-500 shadow-xl shadow-purple-900/60 cursor-pointer group/avatar"
                    onClick={() => avatarInputRef.current?.click()}
                >
                    <div className="w-full h-full rounded-full border-4 border-[#2E1065] overflow-hidden bg-gray-800 relative z-10">
                        <img alt="Professor Avatar" className="w-full h-full object-cover transform transition-transform duration-700 group-hover:scale-110" src={currentUser?.avatar || IMAGES.avatar} />
            ...
            <div className="flex-1 flex flex-col items-center sm:items-start text-center sm:text-left pt-2">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gray-400/20 border border-gray-400/30 backdrop-blur-md text-[11px] font-bold uppercase tracking-widest text-gray-200 mb-3 shadow-sm">
                    <span className="material-icons-round text-sm text-gray-400">military_tech</span>
                    Painel do Professor (Prata)
                </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="relative bg-surface-light dark:bg-surface-dark p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 flex items-center overflow-hidden transition-all hover:scale-[1.02] hover:shadow-xl">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-3 rounded-l-xl overflow-hidden flex flex-col shadow-[2px_0_10px_rgba(0,0,0,0.1)]">
                <div className={`flex-1 ${getBaseRankGradient(currentRank)}`}></div>
                {currentRank.toLowerCase().includes('ponta') && (
                    <div className={`h-1/4 ${getRankGradient(currentRank)}`}></div>
                )}
            </div>
